...
All components, devices and systems that provide SFPF functionality will implement the OpenC2 Actions, Targets, Specifiers and Arguments identified as required in this document. Actions that are applicable, but not necessarily required, for SFPF will be identified as optional.
...
1. The unique name of the SFPF schema is `oasis-open.org/openc2/v1.0/ap-sfpf`
2. The namespace identifier (nsid) referring to the SFPF schema is:  `sfpf`
3. The definitions of and conformance requirements for these types are contained in this document

## 2.1 OpenC2 Command Components
The components of an OpenC2 Command include Actions, Targets, Actuators and associated Arguments and Specifiers. Appropriate aggregation of the components will define a Command-body that is meaningful in the context of an SFPF.

This specification identifies the applicable components of an OpenC2 Command. The components of an OpenC2 Command include:

* Action:  A subset of the Actions defined in the OpenC2 Language Specification that are meaningful in the context of a SFPF.
    ...
    * This profile MAY augment the definition of the Actions in the context of a SFPF
...
* Target:  A subset of the Targets and Target-Specifiers defined in Version 1.0 of the OpenC2 Language Specification that are meaningful in the context of SFPF and one Target (and its associated Specifier) that is defined in this specification
...
* Actuator:  A set of specifiers defined in this specification that are meaningful in the context of SFPF

### 2.1.1 Actions
Table 2.1.1-1 presents the OpenC2 Actions defined in version 1.0 of the Language Specification which are meaningful in the context of an SFPF. The particular Action/Target pairs that are required or are optional are presented in [Section 2.3](#23-openc2-commands).

**Table 2.1.1-1. Actions Applicable to SFPF**
...
### 2.1.2 Targets
Table 2.1.2-1 summarizes the Targets defined in Version 1.0 of the [[OpenC2-Lang-v1.0]](#openc2-lang-v10) as they relate to SFPF functionality. Table 2.1.2-2 summarizes the Targets that are defined in this specification.

#### 2.1.2.1 Common Targets
Table 2.1.2-1 lists the Targets defined in the OpenC2 Language Specification that are applicable to SFPF. The particular Action/Target pairs that are required or are optional are presented in [Section 2.3](#23-openc2-commands).

**Table 2.1.2-1. Targets Applicable to SFPF**
...
#### 2.1.2.2 SFPF Targets
The list of common Targets is extended to include the additional Targets defined in this section and referenced with the sfpf namespace.

**Table 2.1.2-2. Targets Unique to SFPF**
...
Table 2.1.3-1 lists the Command Arguments defined in the [[OpenC2-Lang-v1.0]](#openc2-lang-v10) that are applicable to SFPF.

**Table 2.1.3-1. Command Arguments applicable to SFPF**
...
#### 2.1.3.2 SFPF Arguments
The list of common Command Arguments is extended to include the additional Command Arguments defined in this section and referenced with the sfpf namespace.

**Table 2.1.3-2. Command Arguments Unique to SFPF**
...
| Type Name | Type | Description |
| :--- | :--- | :--- |
| **Friendly-Name** | String | A friendly name associated with a rule |

**_Type: Tags_**

| Type Name | Type | Description |
| :--- | :--- | :--- |
| **Tags** | String | Tags to be associated with a rule |

The semantics/requirements as they relate to SFPF arguments:
...
* insert_rule:
    ...
    * The value MUST be unique within the scope of an Openc2 Producer and an Openc2 Consumer- i.e., the value MUST map to exactly one deny <target> or allow <target> for a given instance of an SFPF
...
An Actuator is the entity that provides the functionality and performs the Action. The Actuator executes the Action on the Target. In the context of this profile, the Actuator is the SFPF and the presence of one or more Specifiers further refine which Actuator(s) shall execute the Action.

Table 2.1.4-1 lists the Specifiers that are applicable to the SFLF Actuator. [Annex A](#annex-a-sample-commands) provides sample Commands with the use of Specifiers.

The Actuator Specifiers defined in this document are referenced under the sfpf namespace.

**Table 2.1.4-1. SFPF Specifiers**

**_Type: Specifiers (Map)_**

| ID | Name | Type | # | Description |
| :--- | :--- | :--- | :--- | :--- |
| 1 | **hostname** | String | 0..1 | [[RFC1123]](#rfc1123) hostname (can be a domain name or IP address) for a particular device with SFPF functionality |
| 2 | **named_group** | String | 0..1 | User defined collection of devices with SFPF functionality |
| 3 | **asset_id** | String | 0..1 | Unique identifier for a particular SFPF |
| 4 | **asset_tuple** | String | 0..10 | Unique tuple identifier for a particular SFPF consisting of a list of up to 10 strings |
...
Table 2.2.1-1 lists the Response Results properties defined in the [[OpenC2-Lang-v1.0]](#openc2-lang-v10) that are applicable to SFPF.

**Table 2.2.1-1. Response Results Applicable to SFPF**
...
Table 2.2.1-2 lists the Response Status Codes defined in the OpenC2 Language Specification that are applicable to SFPF.
...
Table 2.3-1 defines the Commands that are valid in the context of the SFPF profile. An Action (the top row in Table 2.3-1) paired with a Target (the first column in Table 2.3-1) defines a valid Command. The subsequent subsections provide the property tables applicable to each OpenC2 Command.
...
| **features** |   |   | valid |   |   |
| **sfpf:rule_number** |   |   |   | valid |   |
| **file** |   |   |   |   | valid |

Table 2.3-2 defines the Command Arguments that are allowed for a particular Command by the SFPF profile. A Command (the top row in Table 2.3-2) paired with an Argument (the first column in Table 2.3-2) defines an allowable combination. The subsection identified at the intersection of the Command/Argument provides details applicable to each Command as influenced by the Argument.

**Table 2.3-2. Command Arguments Matrix**

|   | Allow _target_ | Deny _target_ | Query features | Delete sfpf:rule_number | Update file |
| :--- | :---: | :---: | :---: | :---: | :---: |
| **response_requested** | [2.3.1](#231-allow) | [2.3.2](#232-deny) | [2.3.3.1](#2331-query-features) | [2.3.4.1](#2341-delete-sfpfrule_number) | [2.3.5.1](#2351-update-file) |
| **start_time** | [2.3.1](#231-allow)| [2.3.2](#232-deny) |   | [2.3.4.1](#2341-delete-sfpfrule_number) | [2.3.5.1](#2351-update-file) |
...
Upon receipt of an unsupported Command Argument, SFPF Consumers
...
OpenC2 Consumers that receive 'allow <target>' Commands and support the 'delete sfpf:rule_number' Command:
...
The 'allow ipv4_connection' Command is OPTIONAL for Openc2 Producers implementing the SFPF.
The 'allow ipv4_connection' Command is OPTIONAL for Openc2 Consumers implementing the SFPF.
...
The 'allow ipv6_connection' Command is OPTIONAL for Openc2 Producers implementing the SFPF.
The 'allow ipv6_connection' Command is OPTIONAL for Openc2 Consumers implementing the SFPF.
...
| :--- | :--- | :--- | :--- |
| Working Draft 1 | 09 AUG 2019 | Everett, Alex | Initial draft |
